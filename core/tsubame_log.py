...
# Tsubame logging
#----------------------------------------------------------------------------
# Copyright 2014, 2017, Martin Kolman
#
...
ANDROID_SPECIAL_LOG_FOLDER = "/sdcard/tsubame_debug_logs"
...
class LogManager(object):
    def __init__(self):
        """Initialize logging for Tsubame.
        ...
        # create main Tsubame logger (root logger)
        self._root_tsubame_logger = logging.getLogger('')
        self._root_tsubame_logger.setLevel(logging.DEBUG)
        # name is undocumented and might blow up in the future ^_-
        self._root_tsubame_logger.name = ""
        ...
        if qrc.is_qrc:
            ...
            if os.path.isdir(ANDROID_SPECIAL_LOG_FOLDER):
                self._root_tsubame_logger.debug("special Android log folder (%s) detected", ANDROID_SPECIAL_LOG_FOLDER)
                self._root_tsubame_logger.debug("enabling early Android log file")
                self.log_folder_path = ANDROID_SPECIAL_LOG_FOLDER
                self.enable_log_file()
                self._log_file_override = True
                self._root_tsubame_logger.debug("early Android log file enabled")
    ...
    def _get_log_filename(self, compression=False):
        if compression:
            return "tsubame_%s.log.gz" % utils.getTimeHashString()
        else:
            return "tsubame_%s.log" % utils.getTimeHashString()
    ...
    def disable_log_file(self):
        with self._log_file_enabled_lock:
            if self._log_file_override:
                ...
                self._root_tsubame_logger.info("not disabling log file due to log file override")
                return
            if self._file_handler:
                self._root_tsubame_logger.info("disabling the log file in: %s", self.log_folder_path)
                self._root_tsubame_logger.info(self.log_file_path)
                self._root_tsubame_logger.removeHandler(self._file_handler)
                ...
                self._log_file_path = None
                self._log_file_compression = False
                self._root_tsubame_logger.info("log file disabled")
    ...
    def enable_stdout_log(self):
        """Enable logging to stdout that has been previously disabled with disable_stdout_log()"""
