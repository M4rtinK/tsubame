...
class ThreadManager(object):
    ...
    def _get_duplicate_name(self, name):
        ...
        index = 1
        while index < 9000:
            thread_name = "%s_%d" % (name, index)
            if thread_name in self._objs:
                index+=1
            else:
                return thread_name
    ...
    def add(self, obj):
        ...
        if obj.name in self._objs:
            obj.name = self._get_duplicate_name(obj.name)
...
class TsubameThread(threading.Thread):
    ...
    def run(self, *args, **kwargs):
        ...
        try:
            if self.target:
                self._conditional_callback(self.target())
        except:
    ...
    def _conditional_callback(self, *args, **kwargs):
...
def init_threading():
    """Set up threading for Tsubame's use."""
    global threadMgr
